(function() {
  ...
  d3.flameGraphUtils = {
    augment: function(node, location) {
      // console.log('Augment:', ...arguments);
      var childSum, children;
      children = node.children;
      if (node.augmented || (node.data && node.data.augmented)) {
        return node;
      }
      ...
      if (!(children != null ? children.length : void 0)) {
        node.augmented = true;
        // console.log('Augment result no children:', node);
        return node;
      }
      ...
      if (childSum < node.value) {
        // console.log('Filled created with value:', node.value - childSum);
        children.push({
      ...
      node.augmented = true;
      // console.log('Augment result:', node);
      return node;
    },
    partition: function(data) {
      console.log('Partition:', ...arguments);
      let res = d3.layout.partition().sort(function(a, b) {
        if (a.filler) {
          return 1;
        }
        if (b.filler) {
      ...
      console.log('Partition result:', res);
      return res;
    },
  ...
  d3.flameGraph = function(selector, root, debug) {
    ...
    FlameGraph = (function() {
      ...
      FlameGraph.prototype.render = function() {
        ...
        console.log('before sL this._data: ', this._data);
        this.x = d3.scale.linear().domain([
          0, d3.max(this._data, function(d) {
            console.log('..scaleLinear max d:', d.x, d.x + d.dx, d);
            return d.x + d.dx;
          })
        ]).range([0, this.width()]);
        console.log('after sL this._data: ', this._data);
        ...
        this.y = d3.scale.quantize().domain([
          d3.max(this._data, function(d) {
            console.log('..scaleQuantize max d:', d.y, d.y + d.dy, d);
            return d.y;
          }), 0
        ]).range(d3.range(maxLevels).map((function(_this) {
          return function(cell) {
            // console.log('..scaleQuantize range cell:', cell);
            console.log('..scaleQuantize range:', cell, ((cell + visibleCells) - (_this._ancestors.length + maxLevels)) * _this.cellHeight());
        ...
        renderNode = {
          x: (function(_this) {
            return function(d) {
              console.log('.. renderNode x:', d.name, d.x, d.x + d.dx, _this.x(d.x), d);
              return _this.x(d.x);
            };
          })(this),
          y: (function(_this) {
            return function(d) {
              console.log('.. renderNode y:', d.name, d.y, d.y + d.dy, _this.y(d.y), d);
              return _this.y(d.y);
            };
          })(this),
      ...
      FlameGraph.prototype._createContainer = function() {
        console.log('CreateContainer:', ...arguments);
        var offset, svg;
      ...
      FlameGraph.prototype._renderNodes = function(containers, attrs, enter) {
        console.log('RenderNodes:', ...arguments);
        ...
        console.log('targetRects before: ', targetRects);
        ...
        console.log('targetRects after: ', targetRects);
        if (!enter) {
      ...
      FlameGraph.prototype._renderTooltip = function() {
        console.log('RenderTooltip:', ...arguments);
      ...
      FlameGraph.prototype._renderAncestors = function() {
        console.log('RenderAncestors:', ...arguments);
      ...
      FlameGraph.prototype._enableNavigation = function() {
        console.log('EnableNavigation:', ...arguments);
        var clickable;
      ...
      FlameGraph.prototype._generateAccessors = function(accessors) {
        console.log('GenerateAccessors:', ...arguments);
